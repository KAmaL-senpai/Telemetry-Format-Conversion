...
def convertFromFormat1(jsonObject):
    ...
    result = {
        ...
        'location': {
            ...
            # extract the area from the location string
            'area': locationParts[2],
            # extract the factory from the location string
            'factory': locationParts[3],
            # extract the section from the location string
            'section': locationParts[4]
        },
        'data': {
            # copy the operationStatus to status
...
def convertFromFormat2(jsonObject):
    ...
    data = datetime.datetime.strptime(
        jsonObject['timestamp'], '%Y-%m-%dT%H:%M:%S.%fZ')  # ISO 8601 format
